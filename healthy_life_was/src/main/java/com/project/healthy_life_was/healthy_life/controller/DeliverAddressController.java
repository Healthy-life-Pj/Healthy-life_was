...
public class DeliverAddressController {
    ...
    private final String DELIVER_ADDRESS_PUT = "/{addressDeliverId}";
    private final String DELIVER_ADDRESS_DELETE = "/delete/{addressDeliverId}";
    ...
    public ResponseEntity<ResponseDto<DeliverAddressResponseDto>> getAddressAll (
            @AuthenticationPrincipal UserDetails userDetails
    ) {
        if (userDetails == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        ...
        String username = userDetails.getUsername();
        ResponseDto<DeliverAddressResponseDto> response = deliverAddressService.getAddressAll(username);
        HttpStatus status = response.isResult() ? HttpStatus.OK : HttpStatus.BAD_REQUEST;
        return ResponseEntity.status(status).body(response);
    }
    ...
    public ResponseEntity<ResponseDto<DeliverAddressResponseDto>> updateAddress (
            @AuthenticationPrincipal UserDetails userDetails,
            @PathVariable Long addressDeliverId,
            @RequestBody DeliverAddressRequestDto dto
    ) {
        if (userDetails == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        ...
        String username = userDetails.getUsername();
        ResponseDto<DeliverAddressResponseDto> response = deliverAddressService.updateAddress(username, dto, addressDeliverId);
        HttpStatus status = response.isResult() ? HttpStatus.OK : HttpStatus.BAD_REQUEST;
        return ResponseEntity.status(status).body(response);
    }
